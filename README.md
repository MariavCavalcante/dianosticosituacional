...
cabeçalho mostra um aviso de "demonstração estática offline"). O mapa
coroplético não carrega a geometria dos municípios nesse modo específico
(o navegador bloqueia esse tipo de requisição em arquivos abertos localmente
por `file://`), mas todos os demais cartões, filtros, busca, gráficos e
tabelas funcionam normalmente. Na versão publicada (Netlify), o mapa funciona
sem essa limitação.
